...
function AdminRampsPage() {
  ...
  if (q.error?.message?.includes("Admin")) {
    return (
      <div className="p-8">
        <Card>
          <CardContent className="p-6">Admin only.</CardContent>
        </Card>
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6 max-w-6xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate({ to: "/admin/ledger" })}>
            Ledger
          </Button>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Master gas (SOL)</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-mono">{gas ? gas.sol.toFixed(4) : "—"}</p>
            {gas && gas.sol < 0.5 && (
              <p className="text-xs text-destructive mt-1">Low — top up master wallet</p>
            )}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Paytrie mode</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Total intents</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-mono">{intents.length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Recent ramp intents</CardTitle>
        </CardHeader>
        <CardContent>
          {q.isLoading ? (
            <p>Loading…</p>
                ...
                <tbody>
                  {intents.map((i) => (
                    <tr key={i.id} className="border-b">
                      ...
                      <td className="py-2 pr-3 font-mono text-xs truncate max-w-[100px]">
                        {i.user_id?.slice(0, 8)}
                      </td>
                      <td className="py-2 pr-3">
                      ...
                      <td className="py-2 pr-3 text-right font-mono">
                        {i.amount_cad ? Number(i.amount_cad).toFixed(2) : "—"}
                      </td>
                      <td className="py-2 pr-3 text-right font-mono">
                        {i.amount_usdc ? Number(i.amount_usdc).toFixed(2) : "—"}
                      </td>
                      <td className="py-2 pr-3 font-mono text-xs truncate max-w-[120px]">
                        {i.paytrie_tx_id}
                      </td>
                      <td className="py-2 pr-3 font-mono text-xs truncate max-w-[120px]">
                        {i.sweep_tx_signature ?? i.payout_tx_signature ?? "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>

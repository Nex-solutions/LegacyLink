...
function Signup() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!form.email.includes("@") || form.password.length < 6)
      return toast.error("Please enter a valid email and a password of 6+ characters.");
    ...
    try {
      ...
      try {
        await provision({ data: undefined } as never);
      } catch (e) {
  ...
  return (
    <AuthSplit quote="The greatest gift you leave behind is certainty.">
      ...
      <p className="mt-2" style={{ color: "var(--warm-gray)" }}>
        Begin protecting what matters in under 10 minutes.
      </p>
      <div
      ...
      >
        <strong style={{ color: "var(--honey)" }}>Test mode:</strong> we pre-filled fake demo
        credentials so you can sign up with one click. Edit any field if you'd like.
      </div>
      ...
      <p className="mt-6 text-sm text-center" style={{ color: "var(--warm-gray)" }}>
        ...
        <Link to="/login" style={{ color: "var(--honey)" }} className="font-medium">
          Sign in
        </Link>
      </p>
      <p className="mt-3 text-xs text-center" style={{ color: "var(--warm-gray)" }}>
        Are you an advisor?{" "}
        <Link to="/advisor/signup" style={{ color: "var(--honey)" }} className="font-medium">

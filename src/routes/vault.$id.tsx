...
import {
  ...
  serverCheckIn,
  serverRelease,
  serverReplaceBeneficiaries,
  serverUpdateCondition,
  serverUpdateLetter,
  serverAddFunds,
  serverEnsureClaimTokens,
...
function CountdownBox({ value, label }: { value: number; label: string }) {
  return (
    <div
      className="ll-card p-5 text-center min-w-[80px]"
      style={{ border: "2px solid var(--honey)" }}
    >
      <div
        style={{
      ...
      <div className="text-xs mt-2 uppercase tracking-wider" style={{ color: "var(--warm-gray)" }}>
        {label}
      </div>
    </div>
  );
...
function VaultDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (shouldRelease(v) && v.status !== "Released") {
        toast.success(
          "This vault just met its release condition. Beneficiaries are being notified.",
        );
      }
  ...
  async function release() {
    ...
    try {
      ...
      console.error(e);
      toast.error("Couldn't release vault");
    } finally {
      setReleasing(false);
    }
  }
  ...
  async function sendPdfs() {
    ...
    try {
      ...
      toast.success(
        `${merged.length} PDF${merged.length === 1 ? "" : "s"} downloaded. Forward each to its beneficiary.`,
      );
    } catch (e) {
      console.error(e);
    ...
    <PageShell>
      ...
      <div className="relative overflow-hidden" style={{ background: "var(--forest)" }}>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 lg:px-12 py-14">
          <Link to="/dashboard" className="text-sm" style={{ color: "rgba(250,250,247,0.75)" }}>
            ← Back to dashboard
          </Link>
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
            ...
            <span
              ...
              style={{
                background: vault.status === "Active" ? "var(--sage)" : "rgba(250,250,247,0.2)",
                color: "var(--forest)",
              }}
            >
              {vault.status}
          ...
          <p className="mt-2" style={{ color: "rgba(250,250,247,0.78)" }}>
            {cond.kind === "time" &&
              `Releases automatically on ${format(parseISO(cond.unlock_date), "MMMM d, yyyy")}`}
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-12 py-12 grid lg:grid-cols-[1.5fr_1fr] gap-8">
        <div className="space-y-6">
          ...
          {cond.kind === "manual" && (
            <div className="ll-card p-8">
              <h3 style={{ fontFamily: "var(--font-serif)", fontSize: 22, fontWeight: 600 }}>
                You control when this releases.
              </h3>
              <p className="mt-2" style={{ color: "var(--warm-gray)" }}>
                This vault waits patiently until you say it's time.
              </p>
              {vault.status === "Active" && (
                <button
                  onClick={() => setShowRelease(true)}
                  className="ll-pill ll-pill-secondary mt-6"
                >
                  Release Vault
                </button>
              )}
          ...
          {vault.status === "Released" && <ReleasedClaimPanel vault={vault} />}
          ...
          <div className="ll-card p-8">
            ...
            <div className="mt-6 relative pl-8">
              <div
                ...
                style={{ background: "rgba(26,46,26,0.15)" }}
              />
              {(() => {
                ...
                const events: { d: string; label: string; sig?: string | null }[] = [
                  ...
                  {
                    d: vault.created_at,
                    label: `Funded with ${formatCAD(vault.amount_cad)}`,
                    sig: v.tx_signature,
                  },
                  ...(cond.kind === "inactivity"
                    ? [{ d: cond.last_checkin, label: "Last check-in" }]
                    : []),
                  ...(vault.status === "Released"
                    ? [
                        {
                          d: new Date().toISOString().slice(0, 10),
                          label: "Released to beneficiaries",
                          sig: v.tx_signature,
                        },
                      ]
                    : []),
                ...
                return events.map((e, i) => (
                  <div key={i} className="relative mb-5">
                    ...
                    <p className="text-xs" style={{ color: "var(--warm-gray)" }}>
                      {format(parseISO(e.d), "MMMM d, yyyy")}
                    </p>
                    {e.sig && (
                      <a
        ...
        <div className="space-y-6">
          <TrusteesPanel
            ...
            onChange={async (b) => {
              try {
                await serverReplaceBeneficiaries(vault.id, b);
                const v = getVault(vault.id);
                if (v) setVault(v);
              } catch (e) {
                console.error(e);
                toast.error("Couldn't update trustees");
              }
            }}
          />
          ...
          {vault.status === "Active" && (
            <ConditionPanel
              ...
              onChange={async (c) => {
                try {
                  await serverUpdateCondition(vault.id, c);
                  const v = getVault(vault.id);
                  if (v) setVault(v);
                } catch (e) {
                  console.error(e);
                  toast.error("Couldn't update condition");
                }
              }}
            />
          )}
            ...
            <div className="mt-6 space-y-3">
              {vault.status === "Active" && (
                <AddFundsButton
                  ...
                  onAdded={async (amt) => {
                    try {
                      await serverAddFunds(vault.id, amt);
                      const v = getVault(vault.id);
                      if (v) setVault(v);
                    } catch (e) {
                      console.error(e);
                      toast.error("Couldn't add funds");
                    }
                  }}
                />
              )}
              {cond.kind === "manual" && vault.status === "Active" && (
                <button
                  onClick={() => setShowRelease(true)}
                  className="ll-pill ll-pill-secondary w-full"
                >
                  Release Now
                </button>
              )}
      ...
      {showRelease && (
        <div
          ...
          style={{ background: "rgba(26,46,26,0.5)" }}
        >
          <motion.div
          ...
          >
            ...
            <p className="mt-3" style={{ color: "var(--warm-gray)" }}>
              This will send funds to your beneficiaries via Interac e-Transfer. This cannot be
              undone.
            </p>
            <div className="flex justify-end gap-3 mt-8">
              <button onClick={() => setShowRelease(false)} className="ll-pill ll-pill-ghost">
                Cancel
              </button>
              <button onClick={release} disabled={releasing} className="ll-pill ll-pill-secondary">
...
function TimeCondition({ unlockDate }: { unlockDate: string }) {
  ...
  return (
    <div className="ll-card p-8">
      ...
      <p className="mt-6" style={{ color: "var(--warm-gray)" }}>
        Unlocks {format(target, "MMMM d, yyyy")}
      </p>
    </div>
  );
}

function InactivityCondition({
  cond,
  onCheckIn,
}: {
  cond: { kind: "inactivity"; inactivity_days: number; last_checkin: string };
  onCheckIn: () => void;
}) {
  ...
  return (
    <div className="ll-card p-8">
      ...
      <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
        Last check-in: {format(last, "MMMM d, yyyy")}
      </p>
      <div className="mt-6 flex items-center gap-8">
        <svg width="170" height="170" viewBox="0 0 170 170">
          ...
          <text
            ...
            y="105"
            textAnchor="middle"
            style={{ fontSize: 11, fill: "var(--warm-gray)" }}
          >
            days left
          </text>
        </svg>
        <div className="flex-1">
          <button onClick={onCheckIn} className="ll-pill ll-pill-sage">
            ✓ Check In
          </button>
          <p className="text-xs mt-3" style={{ color: "var(--warm-gray)" }}>
            Checking in resets your {cond.inactivity_days}-day countdown.
          </p>
        </div>
      </div>
    </div>
...
function TrusteesPanel({
  ...
  function save() {
    if (total !== 100) {
      toast.error(`Allocations must total 100% (currently ${total}%)`);
      return;
    }
    if (list.some((b) => !b.name.trim() || !b.email.trim())) {
      toast.error("Every trustee needs a name and email");
      return;
    }
    onChange(list);
    setEditing(false);
            ...
            <div key={b.email} className="flex items-center gap-4">
              ...
              <div className="text-right">
                ...
                <p className="text-xs mt-1" style={{ color: "var(--warm-gray)" }}>
                  {formatCAD((vault.amount_cad * b.pct) / 100)}
                </p>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="ll-card p-8">
      <div className="flex items-center justify-between gap-3">
        ...
        <span
          className="text-xs"
          style={{ color: total === 100 ? "var(--forest)" : "var(--warm-gray)" }}
        >
          Total: {total}%
        </span>
      </div>
      <div className="mt-6 space-y-4">
        {list.map((b, i) => (
          <div
            ...
            style={{ borderBottom: "1px solid rgba(26,46,26,0.08)" }}
          >
        ...
        <button onClick={add} className="ll-pill ll-pill-ghost w-full text-sm">
          + Add trustee
        </button>
      </div>
      <div className="flex gap-2 mt-6">
        <button onClick={() => setEditing(false)} className="ll-pill ll-pill-ghost flex-1">
          Cancel
        </button>
        <button onClick={save} className="ll-pill ll-pill-secondary flex-1">
          Save
        </button>
      </div>
    </div>
...
function AddFundsButton({ vault, onAdded }: { vault: Vault; onAdded: (amt: number) => void }) {
  ...
  const n = Number(amt);
  const fee = method === "card" ? Math.round(n * 0.029 * 100) / 100 + 0.3 : 0;
  const total = n + fee;
  ...
  function next() {
    if (step === "amount") {
      ...
      if (method === "interac" && !email.trim()) {
        toast.error("Enter the email to send from");
        return;
      }
      if (method === "card") {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="ll-pill ll-pill-sage w-full">
        + Add Funds
      </button>
      {open && (
        <div
          ...
          >
            {step !== "success" && step !== "processing" && (
              <>
                ...
                >
                  Add funds · Step{" "}
                  {step === "amount" ? 1 : step === "method" ? 2 : step === "details" ? 3 : 4} of 4
                </p>
                <h3
                  className="mt-1"
                  style={{ fontFamily: "var(--font-serif)", fontSize: 26, fontWeight: 600 }}
                >
            ...
            {step === "amount" && (
              <div className="mt-6">
                ...
                <p className="text-xs mt-2" style={{ color: "var(--warm-gray)" }}>
                  Topping up <strong>{vault.name}</strong>. Funds can only be added once active.
                </p>
              </div>
            )}
              ...
              <div className="mt-6 space-y-2">
                {(
                  ...
                  <button
                    ...
                    style={{
                      borderColor: method === opt.id ? "var(--honey)" : "rgba(26,46,26,0.15)",
                      background: method === opt.id ? "rgba(218,165,32,0.06)" : "transparent",
                    }}
                  >
                    <span
                      ...
                      style={{
                        ...
                        background: method === opt.id ? "var(--honey)" : "transparent",
                      }}
                    />
                    <div className="flex-1">
            ...
            {step === "details" && method === "interac" && (
              <div className="mt-6">
                ...
                <input
                  ...
                  autoFocus
                  className="mt-2 w-full px-4 py-3 rounded border"
                  style={{ borderColor: "rgba(26,46,26,0.2)" }}
                />
                <p className="text-xs mt-3" style={{ color: "var(--warm-gray)" }}>
                  We'll send a request for {formatCAD(n)} to your bank's Interac.
                </p>
              </div>
            )}
              ...
              <div className="mt-6 space-y-3">
                <div>
                  ...
                  <input
                    ...
                    autoFocus
                    className="mt-1 w-full px-4 py-3 rounded border"
                    style={{ borderColor: "rgba(26,46,26,0.2)" }}
                  />
                </div>
            ...
            {step === "details" && method === "bank" && (
              <div className="mt-6 space-y-2 text-sm" style={{ color: "var(--forest)" }}>
                ...
                <div className="p-4 rounded" style={{ background: "rgba(26,46,26,0.04)" }}>
                  <p>
                    <strong>Institution:</strong> 003 (RBC)
                  </p>
                  <p>
                    <strong>Transit:</strong> 04821
                  </p>
                  <p>
                    <strong>Account:</strong> 1029384756
                  </p>
                  <p>
                    <strong>Reference:</strong> {vault.id.slice(0, 8).toUpperCase()}
                  </p>
                </div>
                <p className="text-xs" style={{ color: "var(--warm-gray)" }}>
                  Funds will appear in {vault.name} within 1–3 business days.
                </p>
              </div>
            )}
            ...
            {step === "processing" && (
              <div className="py-10 text-center">
                <div
                  className="w-12 h-12 mx-auto rounded-full border-4 animate-spin"
                  style={{ borderColor: "rgba(26,46,26,0.1)", borderTopColor: "var(--honey)" }}
                />
                <p
                  className="mt-6"
                  style={{ fontFamily: "var(--font-serif)", fontSize: 22, color: "var(--forest)" }}
                >
                  Processing payment…
                </p>
                <p className="text-xs mt-2" style={{ color: "var(--warm-gray)" }}>
                  Securely charging {formatCAD(total)}
                </p>
              </div>
            )}
            ...
            {step === "success" && (
              <div className="py-8 text-center">
                ...
                <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
                  {formatCAD(n)} added to {vault.name}.
                </p>
                <button onClick={close} className="ll-pill ll-pill-secondary mt-6">
                  Done
                </button>
              </div>
            )}
            ...
            {step !== "processing" && step !== "success" && (
              <div className="flex justify-between gap-3 mt-8">
                <button
                  onClick={step === "amount" ? close : back}
...
function ConditionPanel({
  ...
  vault: Vault;
  onChange: (c: VaultCondition) => void;
}) {
  const [editing, setEditing] = useState(false);
  const [kind, setKind] = useState<VaultCondition["kind"]>(vault.condition.kind);
  const [unlockDate, setUnlockDate] = useState<string>(
    vault.condition.kind === "time"
      ? vault.condition.unlock_date
    ...
    return (
      <div className="ll-card p-8">
        <div className="flex items-center justify-between gap-3">
          ...
          <button onClick={open} className="ll-pill ll-pill-ghost text-sm">
            Change
          </button>
        </div>
        <div className="mt-6 space-y-2 text-sm">
          ...
          <p style={{ color: "var(--warm-gray)" }}>
            {cond.kind === "time" &&
              `Unlocks ${format(parseISO(cond.unlock_date), "MMMM d, yyyy")}`}
  ...
  return (
    <div className="ll-card p-8">
      ...
      <div className="mt-6 space-y-3">
        ...
        {kind === "inactivity" && (
          <div>
            <label
              ...
              style={{ color: "var(--warm-gray)" }}
            >
              Days of silence before release
            </label>
            <input
              type="number"
              min={7}
        ...
        {kind === "manual" && (
          <p className="text-sm" style={{ color: "var(--warm-gray)" }}>
            The vault waits until you release it manually. No timer, no auto-trigger.
          </p>
        )}
      </div>
      ...
      <div className="flex gap-2 mt-6">
        ...
        <button onClick={requestSave} className="ll-pill ll-pill-secondary flex-1">
          Save
        </button>
      </div>
      ...
      {authOpen && (
        ...
        >
          <motion.div
            ...
            className="ll-card p-8 max-w-md w-full"
          >
            <h3 style={{ fontFamily: "var(--font-serif)", fontSize: 24, fontWeight: 600 }}>
              Confirm it's you
            </h3>
            <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
              Changing release conditions is a sensitive action. Re-enter your account password to
              continue.
            </p>
            <input
              type="password"
    ...
    <div className="flex justify-between">
      ...
      <span
        style={{
          ...
          fontFamily: bold ? "var(--font-serif)" : undefined,
        }}
      >
        {value}
      </span>
...
function LegacyLetterPanel({ vault }: { vault: Vault }) {
  ...
  async function downloadSigned() {
    ...
    if (
      !ownerSig.trim() ||
      ownerSig.trim().toLowerCase() !== (u?.name ?? "").trim().toLowerCase()
    ) {
    ...
    if (requiresAdvisor) {
      if (!advisorAck) {
        toast.error("Your advisor must confirm they've reviewed this letter.");
        return;
      }
    ...
    try {
      ...
      const sigs = [
        ...
        ...(requiresAdvisor
          ? [{ name: advisorSig.trim(), role: "Advisor" as const, signedAt: now }]
          : []),
      ];
      ...
      toast.success(
        requiresAdvisor ? "Letter co-signed and downloaded." : "Letter signed and downloaded.",
      );
      setShowSign(false);
    } catch (e) {
      console.error(e);
    ...
    <div className="ll-card p-8">
      ...
      <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
        ...
        {requiresAdvisor && (
          <>
            {" "}
            Requires <strong>your signature and {advisorName}'s</strong> before it can be issued.
          </>
        )}
        {!requiresAdvisor && (
          <>
            {" "}
            Requires <strong>your signature</strong> before it can be issued.
          </>
        )}
      </p>
      <textarea
        ...
        style={{
          borderColor: "rgba(26,46,26,0.2)",
          fontFamily: "var(--font-serif)",
          lineHeight: 1.55,
        }}
      />
      <button onClick={openSign} className="ll-pill ll-pill-secondary w-full mt-4">
      ...
      {showSign && (
        ...
        >
          ...
          >
            <h3 style={{ fontFamily: "var(--font-serif)", fontSize: 24, fontWeight: 600 }}>
              Sign your legacy letter
            </h3>
            <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
              Type your full name to sign.{" "}
              {requiresAdvisor && (
                <>
                  Your linked advisor <strong>{advisorName}</strong> must also co-sign.
                </>
              )}
            </p>
              ...
              <div className="mt-4">
                ...
                >
                  <input
                    type="checkbox"
                    checked={advisorAck}
                    onChange={(e) => setAdvisorAck(e.target.checked)}
                    className="mt-0.5"
                  />
                  <span>
                    {advisorName} has reviewed the conditions, beneficiaries and message above.
                  </span>
                </label>
              </div>
            )}
...
function ReleasedClaimPanel({ vault }: { vault: Vault }) {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(127,168,130,0.08)", border: "1px solid rgba(127,168,130,0.35)" }}
    >
      <div className="flex items-center gap-2">
        <span
        ...
        <h3 style={{ fontFamily: "var(--font-serif)", fontSize: 22, fontWeight: 600 }}>
          Beneficiary claim links
        </h3>
      </div>
      <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
        Open any link below in a private window and use the matching email to walk through the claim
        flow yourself — no second inbox needed.
      </p>
        ...
        {vault.beneficiaries.map((b) => {
          ...
          return (
            <div
              ...
              style={{ background: "rgba(250,250,247,0.6)" }}
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm" style={{ color: "var(--forest)", fontWeight: 500 }}>
                  {b.name}{" "}
                  <span style={{ color: "var(--warm-gray)", fontWeight: 400 }}>· {b.email}</span>
                </p>
                <a
                  href={url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block text-xs font-mono mt-1 truncate underline"
                  style={{ color: "var(--honey)" }}
                >
                  {url}
                </a>
              ...
              <button
                onClick={() => copy(url)}
                className="ll-pill ll-pill-ghost text-xs"

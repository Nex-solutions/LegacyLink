...
import { getThread, markThreadRead, sendMessage, threadId, unreadCount, type Message } from "@/lib/legacy-messages";
...
function FamilyMessages() {
  ...
  const tid = useMemo(() => (user && activeEmail ? threadId(user.email, activeEmail) : null), [user, activeEmail]);
  ...
  return (
    <PageShell>
      ...
      <div className="relative px-6 lg:px-12 pt-6 pb-32 max-w-7xl mx-auto">
        <Blob className="w-[420px] h-[420px] -top-10 -right-20" color="var(--sage)" opacity={0.08} />
        ...
        <div className="relative z-10">
          ...
          <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
            A private channel between you and your advisor. Encrypted in transit, tied to your vault context.
          </p>

          {links.length === 0 ? (
            <div className="mt-10 rounded-2xl p-8 text-center" style={{ background: "rgba(26,46,26,0.04)" }}>
              <p style={{ color: "var(--forest)", fontFamily: "var(--font-serif)", fontSize: 22, fontWeight: 600 }}>
                No advisor linked yet.
              </p>
              <p className="mt-2 text-sm" style={{ color: "var(--warm-gray)" }}>
                Add an advisor from your dashboard to start a conversation.
              </p>
              <button onClick={() => navigate({ to: "/dashboard" })} className="mt-5 ll-pill ll-pill-primary text-sm" style={{ padding: "0.6rem 1.2rem" }}>
                Go to dashboard
              </button>
            </div>
          ) : (
            <div className="mt-8 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-5 rounded-2xl overflow-hidden" style={{ background: "var(--cream)", border: "1px solid rgba(26,46,26,0.08)" }}>
              {/* Sidebar */}
              <aside className="p-3 lg:border-r" style={{ borderColor: "rgba(26,46,26,0.08)", background: "rgba(26,46,26,0.02)" }}>
                <div className="text-[11px] uppercase tracking-[0.18em] px-3 py-2" style={{ color: "var(--warm-gray)" }}>Your advisors</div>
                {links.map(l => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold"
                        style={{ background: "var(--forest)", color: "var(--cream)", fontFamily: "var(--font-serif)" }}>
                        {l.name.split(" ").map(n => n[0]).join("").slice(0, 2)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm truncate" style={{ color: "var(--forest)", fontWeight: 600 }}>{l.name}</div>
                        <div className="text-[11px] truncate" style={{ color: "var(--warm-gray)" }}>
                          {l.status === "connected" ? l.firm || "Linked" : "Pending invite"}
                        </div>
                      </div>
                      {u > 0 && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: "var(--honey)", color: "var(--forest)", fontWeight: 700 }}>{u}</span>
                      )}
                    </button>
                  );
                })}
                    ...
                    <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: "rgba(26,46,26,0.08)" }}>
                      <div>
                        ...
                        <div className="text-[11px]" style={{ color: "var(--warm-gray)" }}>
                          {active.status === "connected" ? "● Linked · read-only access to vaults you grant" : "○ Pending invite"}
                        </div>
                      </div>
